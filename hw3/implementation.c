...
#include "helpers/myfsbitmap.h"
#include "helpers/myfsdisk.h"
...
#define MYFS_DATA ((struct myfs_env *) fuse_get_context()->private_data)
...
void myfs_init(){
    ...
    fuse_get_context()->private_data = &env;
    return;
}
...
int __myfs_getattr_implem(void *fsptr, size_t fssize, int *errnoptr,
	...
	stbuf->st_nlink += dir_node->num_children;
	stbuf->st_mode = dir_node->permissions;
	stbuf->st_uid = dir_node->user_id;
	stbuf->st_gid = dir_node->group_id;
	stbuf->st_atime = dir_node->a_time;
	stbuf->st_mtime = dir_node->m_time;
	stbuf->st_ctime = dir_node->c_time;
	return 0;
}
...
int __myfs_readdir_implem(void *fsptr, size_t fssize, int *errnoptr,
                          const char *path, char ***namesptr, fuse_fill_dir_t filler, struct fuse_file_info *fi, off_t offset) {
    if(!MYFS_DATA->init){
        myfs_init();
    }
    filler(fsptr, ".", NULL, offset );
	filler(fsptr, "..", NULL, offset );
	...
	if(dir_node == NULL){
		...
		for(i = offset; i < dir_node->num_children; i++){
			filler( fsptr, dir_node->children[i]->name, NULL, i );
		}
	}
	return 0;
}
...
int __myfs_mknod_implem(void *fsptr, size_t fssize, int *errnoptr,
                        const char *path) {
    insert_file(path);
	myfstree * node = search_node((char *)path);
	if(node != NULL){
...
int __myfs_rename_implem(void *fsptr, size_t fssize, int *errnoptr,
	...
	if(strcmp(dst->type,"file")==0){
		...
		myfsfile * my_file = find_file((char*)to);
...
int __myfs_truncate_implem(void *fsptr, size_t fssize, int *errnoptr,
	...
	if(my_file != NULL){
		if(fssize <= 0){
			...
			char *buf;
			buf = (char *)malloc(sizeof(char)*(fssize + 1));
			strncpy(buf, my_file->data, fssize);
			free(my_file->data);
			my_file->data = (char *)calloc(fssize + 1, sizeof(char));
...
int __myfs_open_implem(void *fsptr, size_t fssize, int *errnoptr,
                       const char *path) {
    myfstree * my_file_tree_node = search_node((char *)path);
