...
#define ALIGNMENT 16
#define HEADSIZE sizeof(free_blk_header_t)
...
#define NEXT_BLKP2(bp) ((char *)(bp) + GET_SIZE(GET_HEADER(bp)))
#define NEXT_BLKP(sausage)  ((free_blk_header_t *)(sausage))->next
#define PREV_BLKP(sausage) ((free_blk_header_t *)(sausage))->prev
...
void *coalesce(void *sausage){
  ...
  if(prev_alloc && next_alloc){//if both used, do nothing
    ...
    NEXT_BLKP(sausage) = NEXT_BLKP(NEXT_BLKP(sausage)); //point to next sausage
  }
  else if(!prev_alloc && next_alloc){
    //merge with previous block
    size += GET_SIZE(GET_HEADER(PREV_BLKP(sausage)));
    GET_SIZE(GET_HEADER(PREV_BLKP(sausage))) = size;
    NEXT_BLKP(PREV_BLKP(sausage)) = NEXT_BLKP(sausage);
    ...
    GET_SIZE(GET_HEADER(PREV_BLKP(sausage))) = size;
    NEXT_BLKP(PREV_BLKP(sausage)) = NEXT_BLKP(NEXT_BLKP(sausage));
    sausage = PREV_BLKP(sausage);
  }
  return sausage;
}
...
void get_block(size_t rawsize){
  ...
  NEXT_BLKP(sausage) = NEXT_BLKP(first_sausage);
  PREV_BLKP(sausage) = first_sausage;
  NEXT_BLKP(first_sausage) = sausage;
