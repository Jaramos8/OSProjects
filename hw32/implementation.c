 ...
 * @param off offset to the next entry or zero
...
int __myfs_readdir_implem(void *fsptr, size_t fssize, int *errnoptr,
                          const char *path, char ***namesptr, fuse_fill_dir_t filler, struct fuse_file_info *fi) {
    if(!MYFS_DATA->init){
        myfs_init();
    }
    filler(fsptr, ".", NULL, 0 );
	...
	if(dir_node == NULL){
		...
		for(i = 0; i < dir_node->num_children; i++){
			filler( fsptr, dir_node->children[i]->name, NULL, 0 );
		}
	}
	return 0;
}
...
int __myfs_read_implem(void *fsptr, size_t fssize, int *errnoptr,
    ...
    //return pread(fi->fh, fsptr, fssize, offset);
    return *errnoptr;
}
